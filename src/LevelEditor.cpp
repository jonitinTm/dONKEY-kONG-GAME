...
{
    // Match the exact rendered size so picking works at gameplay scale (4×)
    if (_beamTex && _beamTex->id > 0) {
        const float scale = 4.0f;
        return { pos.x, pos.y,
                 _beamTex->width * scale,
                 _beamTex->height * scale };
    }
    ...
    if (_tool == EditorTool::SELECT)
    {
        if (lmbP && !_dragging) {
            if (PickEntity(wm)) {
                ...
                SetStatus(TextFormat("Selected %s #%d  —  drag to move, DEL to delete",
                    ToolName((EditorTool)_sel.type), _sel.index));
            }
            else {
                _sel.clear();
...
void LevelEditor::DrawPlatEnt(const PlatformData& p, bool sel) const
{
    ...
    // ── Textured tiles with per-tile y-shift for tilt ─────────────────────────
    if (_beamTex && _beamTex->id > 0) {
        const float tw = _beamTex->width * scale;
        const float th = _beamTex->height * scale;
        float x = p.x;
        while (x < p.x + p.w) {
            float drawW = fminf(tw, p.x + p.w - x);
            float srcW = drawW / scale;
            float yShift = (x - p.x) * tanf(tiltRad);   // rise along tilt
            DrawTexturePro(*_beamTex,
                { 0, 0, srcW, (float)_beamTex->height },
                { x, p.y + yShift, drawW, th },
                { 0, 0 }, 0.f, WHITE);
            x += tw;
        }
    }
    else {
        // Fallback: plain coloured rect
        DrawRectangle((int)p.x, (int)p.y, (int)p.w, (int)platH,
            { 80, 120, 255, 60 });
    }

    // ── Parallelogram border that matches the tilt ────────────────────────────
    Color bc = sel ? YELLOW : Color{ 80, 120, 255, 200 };
    float lw = sel ? 2.f : 1.5f;
    float yRight = p.w * tanf(tiltRad);          // y-drop at right edge
    float bh = (_beamTex && _beamTex->id > 0)
        ? _beamTex->height * scale
        : platH;

    Vector2 TL = { p.x,       p.y };
    Vector2 TR = { p.x + p.w, p.y + yRight };
    Vector2 BL = { p.x,       p.y + bh };
    Vector2 BR = { p.x + p.w, p.y + yRight + bh };

    DrawLineEx(TL, TR, lw, bc);   // top edge
    DrawLineEx(BL, BR, lw, bc);   // bottom edge
    DrawLineEx(TL, BL, lw, bc);   // left edge
    DrawLineEx(TR, BR, lw, bc);   // right edge
    ...
    // Selection handles at the four actual corners
    if (sel) {
        const float hs = 5.f;
        auto Handle = [hs](Vector2 c) {
            DrawRectangle((int)(c.x - hs), (int)(c.y - hs),
                (int)(hs * 2), (int)(hs * 2), YELLOW);
            };
...
void LevelEditor::DrawLadEnt(const LadderData& l, bool sel) const
{
    ...
    if (_ladderTex && _ladderTex->id > 0) {
        // Tile the ladder texture vertically, same scale as gameplay (4×)
        const float scale = 4.0f;
        const float tileW = 16.f * scale;
        const float tileH = 16.f * scale;
        float drawX = r.x + r.width * 0.5f - tileW * 0.5f;
        for (float y = r.y; y < r.y + r.height; y += tileH) {
            float drawH = fminf(tileH, r.y + r.height - y);
            float srcH = drawH / scale;
            DrawTexturePro(*_ladderTex,
                { 0, 0, 16.f, srcH },
                { drawX, y, tileW, drawH },
                { 0, 0 }, 0.f, WHITE);
        }
...
void LevelEditor::DrawBeamEnt(Vector2 pos, bool sel) const
{
    if (_beamTex && _beamTex->id > 0) {
        const float scale = 4.0f;
        float w = _beamTex->width * scale;
        float h = _beamTex->height * scale;
        DrawTexturePro(*_beamTex,
            { 0, 0, (float)_beamTex->width, (float)_beamTex->height },
            { pos.x, pos.y, w, h },
            { 0, 0 }, 0.f, WHITE);
        if (sel)
            DrawRectangleLinesEx({ pos.x, pos.y, w, h }, 2.f, YELLOW);
    }
    else {
...
void LevelEditor::DrawBackground() const
{
    if (_bgTex && _bgTex->id > 0) {
        // Draw the actual game background scaled to the world size
        DrawTexturePro(*_bgTex,
            { 0, 0, (float)_bgTex->width, (float)_bgTex->height },
            { 0, 0, (float)_sw, (float)_sh },
            { 0, 0 }, 0.f, WHITE);
        // Darken slightly so editor entities are easier to see
        DrawRectangle(0, 0, _sw, _sh, { 0, 0, 0, 60 });
    }
    else {
...
void LevelEditor::DrawPlacementPreview() const
{
    ...
    // ── Drag indicator ─────────────────────────────────────────────────────────
    if (_tool == EditorTool::SELECT && _dragging && _sel.valid()) {
        // Draw a move-arrow cross at mouse position
        const float A = 10.f;
        DrawLineEx({ wm.x - A, wm.y }, { wm.x + A, wm.y }, 2.f, { 255, 255, 0, 200 });
        DrawLineEx({ wm.x, wm.y - A }, { wm.x, wm.y + A }, 2.f, { 255, 255, 0, 200 });
        DrawText("MOVE", (int)wm.x + 12, (int)wm.y - 6, 10, { 255, 255, 0, 200 });
    }
